...
// SMC Call Parameters
#define BAIKAL_PLL_CMU0                       0x20000000
...
{
  UINTN  RegSize = SdPeimRegSize (Reg);
  ...
  )
{
  ...
  Ctrl = SdPeimRegRead (Base, SDHCI_HOST_CONTROL);
  if (On) {
    Ctrl |= SDHCI_CTRL_LED;
  } else {
    Ctrl &= ~SDHCI_CTRL_LED;
  ...
  SdPeimRegWrite (Base, Ctrl, SDHCI_HOST_CONTROL);
  return Status;
}
  ...
  )
{
  ...
  ArmSmcArgs.Arg2 = BAIKAL_SMC_PLAT_CMU_CLKCH_ROUND_RATE;
  ArmSmcArgs.Arg4 = BAIKAL_PLL_CMU0;
...
{
  UINT32  Ctrl  = SdPeimRegRead (Base, SDHCI_HOST_CONTROL);
  UINT32  Ctrl2 = SdPeimRegRead (Base, SDHCI_HOST_CONTROL2);
  ...
  SdPeimRegWrite (Base, Ctrl,  SDHCI_HOST_CONTROL);
  SdPeimRegWrite (Base, Ctrl2, SDHCI_HOST_CONTROL2);
  return EFI_SUCCESS;
}
...
{
  // IRQ
  SdPeimRegWrite (Base, 0xFFFF, SDHCI_INT_STATUS);
  SdPeimRegWrite (Base, 0xFFFF, SDHCI_ERR_STATUS);
  SdPeimRegWrite (Base, 0xFFFF, SDHCI_INT_ENABLE);
  SdPeimRegWrite (Base, 0xFFFF, SDHCI_ERR_ENABLE);
  SdPeimRegWrite (Base, 0x0000, SDHCI_SIGNAL_ENABLE);
  SdPeimRegWrite (Base, 0x0000, SDHCI_ERR_SIGNAL_ENABLE);

  // Config
  SdPeimRegWrite (Base, 0, SDHCI_HOST_CONTROL);
  SdPeimRegWrite (
    Base,
    SDHCI_CTRL_V4_MODE |
  ...
  SdPeimRegWrite (Base, 0, SDHCI_16BIT_BLK_CNT);
  SdPeimRegWrite (Base, 0, SDHCI_32BIT_BLK_CNT);
  SdPeimRegWrite (Base, SDHCI_BLOCK_SIZE_DEFAULT, SDHCI_BLOCK_SIZE);
  SdPeimRegWrite (Base, SDHCI_TIMEOUT_DEFAULT, SDHCI_TIMEOUT_CONTROL);

  // Power
  SdPeimRegWrite (Base, SDHCI_POWER_OFF, SDHCI_POWER_CONTROL);
  SdPeimRegWrite (Base, SDHCI_POWER_330 | SDHCI_POWER_ON, SDHCI_POWER_CONTROL);

  // Clock
  SdPeimRegWrite (Base, SDHCI_CLOCK_EN, SDHCI_CLOCK_CONTROL);
  ...
  )
{
  ...
  SdPeimRegWrite (Base, SDHCI_INT_CARD_INSERT | SDHCI_INT_CARD_REMOVE, SDHCI_INT_STATUS); // BIT6 | BIT7

  // Check Present State Register to see if there is a card presented
  if (SdPeimRegRead (Base, SDHCI_PRESENT_STATE) & SDHCI_CARD_PRESENT) { // BIT16
    return EFI_SUCCESS;
  } else {
    return EFI_NO_MEDIA;
  }
  ...
  )
{
  ...
  default:
    DEBUG ((EFI_D_ERROR, "%a: -- RespType\n", __FUNCTION__));
    Status = EFI_INVALID_PARAMETER;
    goto exit;
  }

  // Present
  Present = BIT0; // CMD Inhibit (CMD)
  if (CmdType  == SdCommandTypeAdtc ||
      RespType == SdResponseTypeR1b ||
      RespType == SdResponseTypeR5b) {
    Present |= BIT1; // CMD Inhibit (DAT)
  }

  // Cmd
  // Busy
  Status = WAIT (SdPeimRegRead (Base, SDHCI_PRESENT_STATE) & Present);
  if (EFI_ERROR (Status)) {
    goto exit;
  }

  // Exec
  SdPeimRegWrite (Base, Blocks,    SDHCI_32BIT_BLK_CNT);
  SdPeimRegWrite (Base, Blocksize, SDHCI_BLOCK_SIZE);
  SdPeimRegWrite (Base, Mode,      SDHCI_TRANSFER_MODE);
  SdPeimRegWrite (Base, Arg,       SDHCI_ARGUMENT);
  SdPeimRegWrite (Base, Cmd,       SDHCI_COMMAND);

  // Response
  Status = WAIT (!(SdPeimRegRead (Base, SDHCI_INT_STATUS) & BIT0)); // SDHCI_INT_RESPONSE
  SdPeimRegWrite (Base, BIT0, SDHCI_INT_STATUS);
  if (EFI_ERROR (Status)) {
    goto exit;
  }

  // Check
  if (SdPeimRegRead (Base, SDHCI_ERR_STATUS)) {
  ...
  if (Len) {
    ...
    for (UINTN J = 0; J < Blocks; J++) {
      if (IsRead) {
        Status = WAIT (!(SdPeimRegRead (Base, SDHCI_INT_STATUS) & BIT5)); // buf_rd_ready, SDHCI_INT_DATA_AVAIL
        SdPeimRegWrite (Base, BIT5, SDHCI_INT_STATUS);
        if (EFI_ERROR (Status)) {
          goto exit;
        }
        ...
        Status = WAIT (!(SdPeimRegRead (Base, SDHCI_INT_STATUS) & BIT4)); // buf_wr_ready
        SdPeimRegWrite (Base, BIT4, SDHCI_INT_STATUS);
        if (EFI_ERROR (Status)) {
          goto exit;
        }
    ...
    // Complete
    Status = WAIT (!(SdPeimRegRead (Base, SDHCI_INT_STATUS) & BIT1)); // xfer_complete, SDHCI_INT_DATA_END
    SdPeimRegWrite (Base, BIT1, SDHCI_INT_STATUS);
    if (EFI_ERROR (Status)) {
      goto exit;
    }

    if (Mode & BIT5) { // Multi Block Select
      if (SdPeimRegRead (Base, SDHCI_32BIT_BLK_CNT)) {
        Status = EFI_DEVICE_ERROR;
        goto exit;
      }
    }
  ...
  )
{
  ...
  // Exec
  *(UINT64 *) (Base + SDHCI_ADMA_ADDRESS) = (UINT64) Buf;
  SdPeimRegWrite (Base, Blocksize, SDHCI_BLOCK_SIZE);
  SdPeimRegWrite (Base, Blocks,    SDHCI_32BIT_BLK_CNT);
  SdPeimRegWrite (Base, Mode,      SDHCI_TRANSFER_MODE);
  SdPeimRegWrite (Base, Arg,       SDHCI_ARGUMENT);
  ...
  // DATA
  if (Len) {
    for (UINTN J = 0; J < EFI_SIZE_TO_PAGES (Len) + 1; J++) {
      Status = WAIT (!(SdPeimRegRead (Base, SDHCI_INT_STATUS) & (BIT1 | BIT3) ));
      if (EFI_ERROR (Status)) {
        goto exit;
      }

      // Block
      if (SdPeimRegRead (Base, SDHCI_INT_STATUS) & BIT3) {
        SdPeimRegWrite (Base, BIT3, SDHCI_INT_STATUS);
        *(UINT64 *) (Base + SDHCI_ADMA_ADDRESS) = *(UINT64 *) (Base + SDHCI_ADMA_ADDRESS);
      }

      // Complete
      if (SdPeimRegRead (Base, SDHCI_INT_STATUS) & BIT1) {
        SdPeimRegWrite (Base, BIT1, SDHCI_INT_STATUS);
        break;
      }
    }
  ...
  )
{
  ...
  return SdPeimCmdExec (Base, CommandIndex, CommandArgument, CommandType, ResponseType, NULL, 0, 0);
}
  ...
  )
{
  ...
  Status = SdPeimCmdExec (Base, CommandIndex, CommandArgument, CommandType, ResponseType, NULL, 0, 0);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  if (SdPeimRegRead (Base, SDHCI_RESPONSE_0) != CommandArgument) {
    return EFI_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  do {
    ...
    Ocr = SdPeimRegRead (Base, SDHCI_RESPONSE_0);
  ...
  )
{
  ...
  Resp[0] = SdPeimRegRead (Base, SDHCI_RESPONSE_0);
  Resp[1] = SdPeimRegRead (Base, SDHCI_RESPONSE_1);
  Resp[2] = SdPeimRegRead (Base, SDHCI_RESPONSE_2);
  Resp[3] = SdPeimRegRead (Base, SDHCI_RESPONSE_3);
  ...
  )
{
  ...
  *Rca = SdPeimRegRead (Base, SDHCI_RESPONSE_0) >> 16;
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    // 4) SD Clock Enable = 0
    Reg  = SdPeimRegRead (Base, SDHCI_CLOCK_CONTROL);
    Reg &= ~SDHCI_CLOCK_CARD_EN;
    SdPeimRegWrite (Base, SDHCI_CLOCK_EN | SDHCI_CLOCK_PLL_EN, SDHCI_CLOCK_CONTROL);

    // 5) Check DAT[3:0] = 0
    Reg = SdPeimRegRead (Base, SDHCI_PRESENT_STATE);
    ...
    Reg = SdPeimRegRead (Base, SDHCI_HOST_CONTROL2);
    SdPeimRegWrite (Base, Reg | SDHCI_CTRL_VDD_180, SDHCI_HOST_CONTROL2);
    ...
    Reg  = SdPeimRegRead (Base, SDHCI_CLOCK_CONTROL);
    Reg |= SDHCI_CLOCK_CARD_EN;
    SdPeimRegWrite (Base, Reg, SDHCI_CLOCK_CONTROL);
  ...
  )
{
  ...
  do {
    ...
    State = (((SdPeimRegRead (Base, SDHCI_RESPONSE_0)) >> 9) & 0xF); // CURRENT_STATE
    MicroSecondDelay (100 * 1000); // 0.1s
    if (!Try--) {
      return EFI_TIMEOUT;
    }
  ...
  Status = SdPeimConfigBusWidth (Base,BusWidth);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  return SdPeimCmdExec (Base, CommandIndex, CommandArgument, CommandType, ResponseType, SwitchResp, Len, IsRead);
}
  ...
  )
{
  ...
  return SdPeimCmdExec (Base, CommandIndex, CommandArgument, CommandType, ResponseType, Buf, Len, IsRead);
}
  ...
  )
{
  ...
  return SdPeimCmdExec (Base, CommandIndex, CommandArgument, CommandType, ResponseType, ExtCSD, Len, IsRead);
}
  ...
  )
{
  ...
  return SdPeimCmdExec (Base, CommandIndex, CommandArgument, CommandType, ResponseType, Buffer, BufferSize, IsRead);
}
...
{
  UINTN  ResponseType    = SdResponseTypeR1;
  UINTN  CommandIndex    = IsRead ? SD_READ_MULTIPLE_BLOCK : SD_WRITE_MULTIPLE_BLOCK; // CMD18 / CMD25
  UINTN  CommandType     = SdCommandTypeAdtc;
  UINTN  CommandArgument = Lba;
  ...
  )
{
  ...
  return SdPeimCmdDmaExec (Base, CommandIndex, CommandArgument, CommandType, ResponseType, Buffer, BufferSize, IsRead);
}
  ...
  )
{
  ...
  // Config
  Status = SdPeimSetClock (ClockFreq);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = WAIT (!(SdPeimRegRead (Base, SDHCI_CLOCK_CONTROL) & SDHCI_CLOCK_STABLE));
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Reg |= SDHCI_CLOCK_CARD_EN;
  SdPeimRegWrite (Base, Reg, SDHCI_CLOCK_CONTROL);
  SdPeimRegWrite (Base, SDHCI_RESET_CMD,  SDHCI_SOFTWARE_RESET);
  SdPeimRegWrite (Base, SDHCI_RESET_DATA, SDHCI_SOFTWARE_RESET);
  MicroSecondDelay (1000);
  Status = WAIT (SdPeimRegRead (Base, SDHCI_SOFTWARE_RESET));
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  UINT32      Clk;
  EFI_STATUS  Status;
  Status = WAIT (!(SdPeimRegRead (Base, SDHCI_PRESENT_STATE) & (BIT0 | BIT1)));
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Clk  = SdPeimRegRead (Base, SDHCI_CLOCK_CONTROL);
  Clk &= ~BIT2; // SDHCI_CLOCK_CARD_EN
  SdPeimRegWrite (Base, Clk, SDHCI_CLOCK_CONTROL);
  return EFI_SUCCESS;
}
...
{
  UINTN  HostCtrl1 = SdPeimRegRead (Base, SDHCI_HOST_CONTROL);
  ...
  SdPeimRegWrite (Base, HostCtrl1, SDHCI_HOST_CONTROL);
  return EFI_SUCCESS;
}

STATIC
EFI_STATUS
SdPeimSwitchBusWidthSd (
  IN  EFI_PHYSICAL_ADDRESS  Base,
  IN  UINTN                 Rca,
  IN  UINTN                 BusWidth
  )
{
  ...
  Status = SdPeimSetBusWidth (Base, Rca, BusWidth); // ACMD6
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = SdPeimSendStatus (Base, Rca, &DevStatus); // CMD13
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  Current    = SdPeimRegRead (Base, SDHCI_MAX_CURRENT);
  ...
  Capabilities = SdPeimRegRead (Base, SDHCI_CAPABILITIES);
  ...
  )
{
  ...
  ControllerVer = SdPeimRegRead (Base, SDHCI_HOST_VERSION) & SDHCI_SPEC_VER_MASK;
  ...
  )
{
  ...
  Status = SdPeimReset (Base); // CMD0
  if (EFI_ERROR (Status)) {
    return Status;
  }

  // 2) Send Cmd8 to the device
  Status = SdPeimVoltageCheck (Base, 0x1, 0xFF); // CMD8
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Try  = 10;
  do {
    Status = SdPeimSendOpCond (Base, Rca, Ocr, S18r, Xpc, Hcs, &Ocr); // ACMD41
    if (EFI_ERROR (Status)) {
      return Status;
    }
  ...
  Status = SdPeimAllSendCid (Base,  &Cid); // CMD2
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = SdPeimSetRca (Base,  &Rca); // CMD3
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = SdPeimGetCsd (Base, Rca, &Csd); // CMD9
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = SdPeimGetCid (Base, Rca, &Cid); // CMD10
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = SdPeimSelect (Base, Rca); // CMD7
  if (EFI_ERROR (Status)) {
    return Status;
  }

  // Bus
  Status = SdPeimSwitchBusWidthSd (Base, Rca, 4); // ACMD6, CMD13
  if (EFI_ERROR (Status)) {
    return Status;
  }

  // Speed
  Status = SdPeimSpeedMode (Base, SD_DEFAULT);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  // Size
  if (TotalSize != NULL) {
    *TotalSize = SdPeimCalcCapacity (&Csd, NULL);
  }
  ...
  )
{
  ...
  Status = SdPeimGetOcrMmc (Base); // CMD1
  if (EFI_ERROR (Status)) {
  goto exit;
  }
  ...
  // ExtCsd:
  Status = SdPeimGetExtCsd (Base, ExtCsd); // CMD8
  if (EFI_ERROR (Status)) {
  goto exit;
  }
  ...
  // Bus
  Status = SdPeimSwitchBusWidthMmc (Base, Rca, 8);
  if (EFI_ERROR (Status)) {
  return Status;
  }
  ...
  // Size
  if (TotalSize != NULL) {
  *TotalSize = SdPeimCalcCapacity (&Csd, ExtCsd);
  }
  return EFI_SUCCESS;
  ...
  )
{
  ...
  Status = SdPeimIdentificationMmc (Base, TotalSize);
  if (!EFI_ERROR (Status)) {
    goto done;
  }

  Status = SdPeimIdentificationSd (Base, TotalSize);
  if (!EFI_ERROR (Status)) {
    goto done;
  }

  Status = SdPeimIdentificationSdio (Base, TotalSize);
  if (!EFI_ERROR (Status)) {
    goto done;
  }
