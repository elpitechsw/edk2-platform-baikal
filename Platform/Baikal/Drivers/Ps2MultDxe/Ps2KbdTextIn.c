...
/**
    ...
    @param Dev                   Ps2 Keyboard private structure
...

{
  ...
  if (Dev->KeyboardErr) {
    ...
    Status = PopEfikeyBufHead (&Dev->EfiKeyQueue, KeyData);
    //
...
/**
  Perform keyboard initialization which implement SIMPLE_TEXT_IN.Reset()
  ...
  )
{
  ...
  Dev = PS2MULT_DEV_FROM_SIMPLE_TEXT_INPUT (This);
  if (Dev->KeyboardErr) {
  ...
  //
  Status = InitKeyboard (Dev, ExtendedVerification);
  if (EFI_ERROR (Status)) {
    return EFI_DEVICE_ERROR;
  }

  Dev->KeyboardEnabled = TRUE;
  ...
  )
{
  ...
  while (1) {
    ...
    //
    Status = KeyboardReadKeyStrokeWorker (Dev, &KeyData);
    if (EFI_ERROR (Status)) {
      return Status;
    }
    //
  ...
  )
{
  ...
  if (!Dev->KeyboardErr) {
    ...
    // next key from the queue
    //
    while (!IsEfikeyBufEmpty (&Dev->EfiKeyQueue)) {
      CopyMem (
        &KeyData,
        &(Dev->EfiKeyQueue.Buffer[Dev->EfiKeyQueue.Head]),
        sizeof (EFI_KEY_DATA)
        );
      if (KeyData.Key.ScanCode == SCAN_NULL && KeyData.Key.UnicodeChar == CHAR_NULL) {
        PopEfikeyBufHead (&Dev->EfiKeyQueue, &KeyData);
        continue;
      }
      //
...

{
  ...
  Dev = PS2MULT_DEV_FROM_SIMPLE_TEXT_INPUT_EX (This);

  return Dev->ConIn.Reset (&Dev->ConIn, ExtendedVerification);
}
...

{
  ...
  Dev = PS2MULT_DEV_FROM_SIMPLE_TEXT_INPUT_EX (This);
  return KeyboardReadKeyStrokeWorker (Dev, KeyData);
}
  ...
  )
{
  ...
  //
  for (Link = Dev->NotifyList.ForwardLink; Link != &Dev->NotifyList; Link = Link->ForwardLink) {
    CurrentNotify = CR (
                      Link,
  ...
  )
{
  ...
  //
  NotifyList = &Dev->NotifyList;
  while (TRUE) {
    //
    // Enter critical section
    //  
    OldTpl = gBS->RaiseTPL (TPL_NOTIFY);
    Status = PopEfikeyBufHead (&Dev->EfiKeyQueueForNotify, &KeyData);
    //
    // Leave critical section
    //
    gBS->RestoreTPL (OldTpl);
