  ...
  )
{
  ...
  DEBUG ((EFI_D_ERROR, "Ps2Mult: %s", ErrMsg));
}
...

{
  ...
  //
  for (TimeOut = 0; TimeOut < KEYBOARD_TIMEOUT; TimeOut += PS2MULT_RX_DELAY) {
    HasData = KeyReadData (Dev, Data);
    if (HasData) {
      break;
    }

    MicroSecondDelay (PS2MULT_RX_DELAY);
  }
  ...
  )
{
  ...
  MicroSecondDelay (PS2MULT_TX_DELAY);
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    DEBUG ((EFI_D_INFO, "Ps2Mult: ID %02X%02X\r\n", Data[0], Data[1]));
    if ((Data[0] == 0xAB) &&
    ...
    DEBUG ((EFI_D_INFO, "Ps2Mult: ID %02X\r\n", Data[0]));
    Status = EFI_NOT_FOUND;
  }
  ...
  )
{
  ...
  if ((Dev->LeftCtrl || Dev->RightCtrl) &&
    ...
    DEBUG ((EFI_D_INFO, "Ps2Mult: resetting system\r\n"));
...
Done:
  ...
  if (!EFI_ERROR (Status)) {
    DEBUG ((EFI_D_INFO, "Ps2Mult: keyboard reset complete\r\n"));
    return EFI_SUCCESS;
  } else {
