...
EFI_STATUS
EFIAPI
BaikalI2cDriverBindingSupported (
  ...
  )
{
  ...
  if (CompareGuid (Dev->Type, &gBaikalNonDiscoverableI2cMasterGuid)) {
    Status = EFI_SUCCESS;
  } else {
...
EFI_STATUS
EFIAPI
BaikalI2cDriverBindingStart (
  IN EFI_DRIVER_BINDING_PROTOCOL  *This,
  IN EFI_HANDLE                   ControllerHandle,
  IN EFI_DEVICE_PATH_PROTOCOL     *RemainingDevicePath OPTIONAL
  )
{
  return BaikalI2cInit (This->DriverBindingHandle, ControllerHandle);
}
...
{
  return BaikalI2cRelease (This->DriverBindingHandle, ControllerHandle);
}


STATIC EFI_DRIVER_BINDING_PROTOCOL  gBaikalI2cDriverBinding = {
  BaikalI2cDriverBindingSupported,
  BaikalI2cDriverBindingStart,
  BaikalI2cDriverBindingStop,
  0xa,
  ...
  )
{
  ...
  Status = EfiLibInstallDriverBindingComponentName2 (
             ImageHandle, SystemTable, &gBaikalI2cDriverBinding, ImageHandle,
             NULL, &gBaikalI2cDriverComponentName2);
  ...
  )
{
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    if (!EFI_ERROR (Status) &&
        CompareGuid (Dev->Type, &gBaikalNonDiscoverableI2cMasterGuid)) {
